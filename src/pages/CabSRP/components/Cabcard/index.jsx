...
function Cabcard({name ,door,classes,user,luggage,cooling,type,location,img,dealCount,basePrice,finalPrice,cancellationType}) {

  const base ='flex flex-col sm:flex-row justify-center h-auto bg-white shadow-lg border-2 border-gray-300 rounded-xl divide-y sm:divide-x divide-gray-300 w-full sm:w-auto';
    ...
    <div className= {`${base}`} >
      <div className='flex flex-col justify-between p-4  md:w-1/3'>
        <div>
          ...
          <p>or similar . {door} doors .{classes}</p>
        </div>
        <div className="flex flex-wrap gap-1 md:w-56 w-full">
          <span className= {`${features}`}><User /> {user}</span>
          <span className={`${features}`}><Luggage /> {luggage}</span>
          <span className={`${features}`}><Cool /> {cooling}</span>
          <span className={`${features}`}><Type /> {type}</span>
        </div>
        <p className='flex gap-2 items-center'><Location />{location}</p>
      </div>
      ...
      <div className='flex flex-col justify-between px-6 py-3 md:w-1/3'>
        <div>
          ...
          <p className='font-semibold text-lg'>{finalPrice} <sub>total</sub></p>
          <span className='text-md flex items-center gap-2'>
            <Cab /> {cancellationType}
          </span>
        </div>
        <button className='bg-blue-950 text-white p-3 mt-2 rounded-lg text-sm sm:text-md font-semibold'>View Details</button>
      </div>
    </div>
  );
}
